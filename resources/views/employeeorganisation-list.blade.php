  ...
  <body>
    ...
    padding-top: 4px; "href="{{url('employerdashboard')}}" data-toggle="tooltip" data-placement="bottom" title="Main Dashboard"><img src="{{ asset('assets/img/home.png')}}" alt="" width="19"></a>
						  <h3>
						      ...
						      <img  style="width: 60px;" src="{{ asset($Roledata->logo) }}"> @endif <h4 style="margin-bottom: 18px;font-size: 21px;
    color: #3290f2;">{{ $Roledata->com_name }}</h4></h3>
						 
						  <h4 style="font-size:15px;    border-bottom: 1px solid #c9c6c6;
    padding-bottom: 10px;">Sign-in to Employee account</h4></div>
			...
			@endif	
							...
							<div class="col-md-12">
							    <select  class="form-control" required name="com" id="com">
							        <option value="">Select Employee For Proceed</option>
							        
							   
							
 @foreach($users as $dept)
 <?php
$ff= DB::table('employee')->where('emp_code','=',$dept->employee_id)->where('emid','=',$dept->emid)->first();
 ?>
  
                     <option value="{{$dept->employee_id}}" >{{$ff->emp_fname}} {{$ff->emp_mname}} {{$ff->emp_lname}} ({{$dept->employee_id}})</option>
                       @endforeach
                        </select>
    ...
    color: #fff;
    background: #277fbe !important;" type="submit">LOGIN</button>
