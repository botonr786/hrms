...
class LandingController extends Controller
{
    ...
    public function Doregister(Request $request)
    {
        ...
        if ($request->p_no != '' && substr($request->p_no, 0, 2) == '83') { // case-insensitive here
            Session::flash('message', 'Invalid input');
            return redirect('register');
        }
        //dd(stripos("83", $request->p_no));
        if ($request->pass != $request->con_password) {
            Session::flash('message', 'Password and Confirm Password both not same');
            ...
            if (empty($Employee)) {
                ...
                Session::flash('message', 'Thank you for registration.Please check your email for next steps.');
    ...
    public function Dashboard(Request $request)
    {
        ...
        if ($request->input('emp_email')) {
            ...
            $Employee1 = DB::table('users')->where('email', '=', base64_decode($request->input('emp_email')))
                ->where('user_type', '!=', 'admin')->where('status', '=', 'active')->first();
    ...
    public function Doforgot(Request $request)
    {
        ...
        $Employee = DB::table('users')->where('email', '=', $request->email)->where('status', '=', 'active')->where('user_type', '!=', 'admin')->first();
        ...
        if (!empty($Employee)) {
            ...
            $data = array('pass' => $Employee->password, 'name' => $Employee->name);
            $toemail = $request->email;
                ...
                if ($Employee->user_type == 'employer') {
                    ...
                    if($subsCnt>0){
                        ...
                        if(!empty($subscription)){
                            ...
                            Session::flash('message', 'Your subscription plan expired!! Renew to continue.');
                            return redirect('/');
                        }
                    ...
                    if($subsCnt>0){
                        ...
                        if(!empty($subscription)){
                            ...
                            Session::flash('message', 'Your organisation subscription plan expired!! Try after renewal.');
                            return redirect('/');
                        }
    ...
    public function DoLoginuser(Request $request)
    {
        ...
        $Employee = DB::table('users_admin_emp')->where('login_id', '=', $request->email)->where('password', '=', $request->psw)->where('status', '=', 'active')->where('user_type', '=', 'user')->first();
        // dd($Employee);
        if (!empty($Employee)) {
    ...
    public function indexserorganisation()
    {
        if (!empty(Session::get('admin_userpp_email'))) {
            ...
            $data['users'] = DB::table('role_authorization_admin_organ')->where('member_id', '=', $member)
                ->where('status', '=', 'active')
                ->get();
    ...
    public function DoLoginorganisation(Request $request)
    {
        if (!empty(Session::get('admin_userpp_email'))) {
            ...
            $Employee = DB::table('users')->where('employee_id', '=', $request->com)->where('status', '=', 'active')->where('user_type', '=', 'employer')->first();
