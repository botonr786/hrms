...
class EmployeeCornerOrganisationController extends Controller
{
	...
	{ 
      ...
      if(!empty($email))
      {
          $data['Roledata'] = DB::table('registration')      
            ...
            $data['users']=DB::table('users')->where('emid','=',$Roledata->reg)
	...
	{   $email = Session::get('emp_email'); 
      if(!empty($email))
      {
          $employer = DB::table('registration')      
                 ->where('status','=','active')  

                  ->where('email','=',$email) 
                  ->first();
         $Employee=DB::table('users')->where('employee_id','=',$request->com)->where('status','=','active')
		 ->where('user_type','=','employee') 
		 ->where('emid','=',$employer->reg)->first();
  
            if (!empty($Employee))
           {
		
            $Roledata = DB::table('users')->where('employee_id','=',$request->com)->where('status','=','active')
			->where('user_type','=','employee')  ->where('emid','=',$employer->reg)
			->first();
				  if(!empty($Roledata)){
				      
		$Roledatadd = DB::table('registration')      
                 ->where('status','=','active')  
                ...
                Session::put('emp_pass_new',$request->psw); 
				   ...
				   return redirect()->intended('employeecornerorganisationdashboard');
 ...
 public function viewdetadash()
    {
        ...
        if(!empty(Session::get('emp_email')))
      {
		 ...
		 $users=DB::table('users')->where('id','=',$user_id)->first();
        $data['employee']=DB::table('employee')->where('emp_code','=',$users->employee_id)->where('emid','=',$users->emid)->first();
        $data['employee_pay_structure']=DB::table('employee_pay_structure')->where('employee_code','=',$users->employee_id)->where('emid','=',$users->emid)->first();
...
$emppaygr=DB::table('pay_scale_master')->where('emid','=',$Roledata->reg )->where('payscale_code','=',$data['employee_rs'][0]->emp_group_name )->first();
   ...
   $data['currency_master']=DB::table('currency_master')->where('emid','=',$Roledata->reg )->get();
   $data['tax_master']=DB::table('tax_master')->where('emid','=',$Roledata->reg )->get();
    ...
    public function viewdetaholiday() {
           ...
           if(!empty(Session::get('emp_email')))
      {
        ...
        return view('employee-corner-organisation/holiday-calendar', compact('holidays'));
      }
       else
       {
              return redirect('/');
...
$holiday_array= array();
       foreach ($holiday_rs as $holiday) 
       {
       	...
       	// dd($holiday_array);
	   return view('employee-corner/apply-leave', compact('leave_type_rs','employee','holiday_array'));
      }
       else
       {
              return redirect('/');
	...
	public function saveApplyLeaveData(Request $request)
	{
	      if(!empty(Session::get('emp_email')))
      {
       	...
       	if($request->leave_inhand >=$days)
        {
			...
			$data['date_of_apply']= date('Y-m-d',strtotime($request->date_of_apply));
		...
		
		{
                   ...
                   $leave_allocation_rs=DB::table('attandence')
				      ->join('employee','attandence.employee_code','=','employee.emp_code')
                           ->where('attandence.employee_code','=',$users->employee_id)
		...
		{$f=1;
			foreach($leave_allocation_rs as $leave_allocation)
			{
				...
				<td>'.$f.'</td>
													...
													<td>'.date('d/m/Y',strtotime($leave_allocation->date)).'<br>('.date('l',strtotime($leave_allocation->date)).')</td>
		...
		public function viewchangecircumstances()
    {
      ...
      $data['employeet']=DB::table('employee')->where('emp_code','=',$users->employee_id)->where('emid','=',$users->emid)->first();
        ...
        return view('employee-corner-organisation/change-of-circumstances',$data);
      }
       else
       {
              return redirect('/');
		...
		public function viewchangecircumstancesedit()
    {
          if(!empty(Session::get('emp_email')))
      {
         $user_id = Session::get('users_id');
		 $users=DB::table('users')->where('id','=',$user_id)->first();
        $data['employee_rs']=DB::table('employee')->where('emp_code','=',$users->employee_id)->where('emid','=',$users->emid)->first();
     
 $data['nation_master']=DB::table('nationality_master')->where('emid','=',$users->emid )->get();
      $data['currency_user']=DB::table('currencies')->orderBy('country', 'asc')->get();
...
'nationality'=> $request->nationality,
 ...
 'emp_dob'=>date('Y-m-d',strtotime($request->emp_dob)),
  ...
  DB::table('employee')
           ->where('emp_code','=',$users->employee_id)->where('emid','=',$users->emid)
            ->update($dataimgper);
...
public function viewemployeeagreement()
  ...
  if(!empty(Session::get('emp_email')))
      {
       ...
       $data['employee_rs']=DB::table('employee')->where('emp_code','=',$users->employee_id)->where('emid','=',$users->emid)->get();
  ...
  $first_day_this_year = date('Y-01-01'); 
       ...
       $emjob=DB::table('employee')->where('emp_code','=',$users->employee_id)->where('emid','=',$users->emid)->first();
	   $email = Session::get('emp_email');
		$Roledata = DB::table('registration')      
                 ->where('status','=','active')  
                  ->where('email','=',$email) 
...
if($request->has('docu_nat')){
 ...
 DB::table('employee_upload')->insert($dataupload);
    ...
    Session::flash('message','Employee data has been successfully updated');
            return redirect('employee-corner/update-profile?q='.$id);
}
