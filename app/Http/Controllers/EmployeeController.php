...
class EmployeeController extends Controller
{
    public function getamployeedas()
    {
        if (!empty(Session::get('emp_email'))) {
            ...
            $data['employee_migarnt'] = DB::table('users')->join('employee', 'users.employee_id', '=', 'employee.emp_code')
                ->where('employee.emid', '=', $Roledata->reg)
                ->where('users.emid', '=', $Roledata->reg)
                ->where('users.status', '=', 'active')
                ->where('users.user_type', '=', 'employee')
...
//employee end
    ...
    public function viewAddEmployeereportnewsenmail($comp_id, $emp_id)
    {
        ...
        if (!empty($email)) {
            ...
            $dataup = array('Roledata' => $Roledata, 'employeedata' => $employeedata);
            $toemail = $employeedata->emp_ps_email;

            Mail::send('mailemployemigrsend', $dataup, function ($message) use ($toemail) {
    ...
    public function viewAddEmployee(Request $request)
    {
        if (!empty(Session::get('emp_email'))) {
            ...
            if ($id) {
                ...
                $data['education_master'] = DB::table('education_master')->get();
                $data['department'] = DB::table('department')->get();
                $data['designation'] = DB::table('designation')->get();
                // $data['employee_type_master'] = DB::table('employee_type_master')->get();
...
";
    ...
    {
        return Excel::download(new EmployeeBulkDataTableExport(null,null,null), 'employees.xlsx');
    }
    ...
    public function generateReport(Request $request)
    {
        ...
        return view('employee/generate-report', compact('departments', 'grades', 'designations'));
    }
    ...
    public function downloadEmployeeReport(Request $request)
    {
        ...
        $export = new EmployeeBulkDataTableExport($selectedDepartments,$selectedGrades,$selectedDesignations);
    ...
    public function update(Request $request, $id)
    {
        ...
        DB::table('employee_bulk_data')->where('id', $id)->update($employeeData);
    ...
    {
        DB::table('employee_bulk_data')->where('id', $id)->delete();
        return redirect('employees');
    }
        ...
        if (!empty(Session::get('emp_email'))) {
            ...
            if ($id) {
                ...
                if ($request->has('visaback_doc')) {
                    ...
                    $file_visa_doc = $request->file('visaback_doc');
                    $extension_visa_doc = $request->visaback_doc->extension();
                    $path_visa_backdoc = $request->visaback_doc->store('employee_vis_doc', 'public');
    ...
    public function savechangecircumstancesedit(Request $request)
    {
        if (!empty(Session::get('emp_email'))) {
            ...
            $employeet = DB::table('employee')->where('emp_code', '=', $employee_code)
            ...
            if (isset($employeetcircumnew->emp_mname) && $employeetcircumnew->emp_mname != '' && $employeetcircumnew->emp_mname != null) {
                $emp_name_history = $emp_name_history . ' ' . $employeetcircumnew->emp_mname;
            } else {
                $emp_name_history = $emp_name_history . ' ' . $employeetnew->emp_mname;
            }
            if (isset($employeetcircumnew->emp_lname) && $employeetcircumnew->emp_lname != '' && $employeetcircumnew->emp_lname != null) {
            ...
            for ($m = date('Y', strtotime($employeet->emp_doj)); $m <= $endtyear; $m++) {
                ...
                if (count($leave_allocation_rs) != 0) {
                    ...
                    foreach ($leave_allocation_rs as $leave_allocation) {
                        ...
                        $employeetemployeeother = DB::table('circumemployee_other_doc')
                            ->where('emp_code', '=', $leave_allocation->emp_code)
                            ->where('emid', '=', $Roledata->reg)
													...
													<td>' . $leave_allocation->emp_ps_phone . '</td>
														...
														<td>' . $dataeotherdoc . '</td>
															...
															<td>' . date('d/m/Y', strtotime($anual_date)) . ' &nbsp &nbsp <a href="' . env("BASE_URL") . 'dashboard/change/' . base64_encode($employee_code) . '/' . base64_encode($anual_date) . '" target="_blank"><i class="fas fa-eye" ></i></a>
															 ...
															 &nbsp &nbsp <a href="' . env("BASE_URL") . 'employee/changesendlet/' . base64_encode($employee_code) . '/' . base64_encode($anual_date) . '" ><i class="fas fa-paper-plane" ></i></a></td>
                        ...
                        $data['result'] .= '<tr>
													...
													<td></td>
														<td></td>
															...
															<td></td>
															<td></td>
															<td>' . date('d/m/Y', strtotime($anual_date)) . ' &nbsp &nbsp <a href="' . env("BASE_URL") . 'dashboard/change/' . base64_encode($employee_code) . '/' . base64_encode($anual_date) . '" target="_blank"><i class="fas fa-eye" ></i></a>&nbsp &nbsp <a href="' . env("BASE_URL") . 'employee/changesendlet/' . base64_encode($employee_code) . '/' . base64_encode($anual_date) . '" ><i class="fas fa-paper-plane" ></i></a></td>
...
<br>
    ...
    public function savenewEmployee(Request $request)
    {
        ...
        if (empty($Employee)) {
            ...
            $deptnmdb = DB::table('payment_type_master')->where('pay_type', '=', $request->emp_payment_type)->where('emid', $request->emid)->first();

            if (empty($deptnmdb)) {
                DB::table('payment_type_master')->insert($datapaytyy);

            }
            $deptnmdhjkb = DB::table('payment_type_master')->where('pay_type', '=', $request->emp_payment_type)->where('emid', $request->emid)->first();
            $st_date = '';
    ...
    public function viewchangecircumstanceseditadd()
    {
        if (!empty(Session::get('emp_email'))) {
            ...
            $Roledata = DB::table('registration')->where('status', '=', 'active')
                ...
                ->where('email', '=', $email)
                ->first();
            $data['employee_rs'] = DB::table('change_circumstances')->where('emid', '=', $Roledata->reg)->orderBy('id', 'ASC')->get();
    ...
    public function viewchangecircumstanceseditnew()
    {
        if (!empty(Session::get('emp_email'))) {
            ...
            $Roledata = DB::table('registration')->where('status', '=', 'active')
                ...
                ->where('email', '=', $email)
                ->first();
            $data['employee_rs'] = DB::table('employee')->where('emid', '=', $Roledata->reg)
                ->where(function ($query) {
    ...
    public function savechangecircumstanceseditnewsave(Request $request)
    {
        if (!empty(Session::get('emp_email'))) {
            ...
            if ($request->has('pr_add_proof')) {
                ...
                DB::table('employee')
                    ->where('emp_code', '=', $request->emp_code)->where('emid', '=', $Roledata->reg)
                    ->update($dataimgper);
    ...
    public function viewAddchangeew($comp_id)
    {
        if (!empty(Session::get('emp_email'))) {
            ...
            $data['employee_changers'] = DB::table('change_circumstances')
                ->where('emid', '=', $Roledata->reg)->where('id', '=', $comp_id)->first();

            $data['employee_newrs'] = DB::table('employee')
                ->where('emid', '=', $Roledata->reg)->where('emp_code', '=', $data['employee_changers']->emp_code)->first();
            ...
            //->where('department_code', '=', $emp_department_code->department_code)
                ->where('designation_status', '=', 'active')->get();

            //dd($emp_department_code);

            $data['employee_otherd_doc_rs'] = DB::table('circumemployee_other_doc')
                ->where('emid', '=', $Roledata->reg)
    ...
    public function saveAddchangegynew(Request $request)
    {
        if (!empty(Session::get('emp_email'))) {
            ...
            DB::table('circumemployee_other_doc')->where('cir_id', '=', $request->newid)->where('emid', '=', $Roledata->reg)->delete();
    ...
    public function viewAddEmployeereportnewexcel($comp_id, $emp_id)
    {
        ...
        if (!empty(Session::get('emp_email'))) {
            ...
            return Excel::download(new ExcelFileExportemployeeInformation($Roledata->reg, base64_decode($emp_id)), 'employeereportexcel.xlsx');
